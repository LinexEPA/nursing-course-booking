(() => {
  ...
  const TEXT_RULES = [
    {
      id: 'T01', type: '明顯文字錯誤', level: 'low', approval: false,
      test: t => has(t, '的的|到時侯|提除|一起病人安全事件|在多點細心|在跟下一班'),
      reason: '偵測到可辨識的錯字、重複字或常見誤植；原則上不因作文式校正退件。',
      suggestion: '不退件｜可直接文字修正或保留原紀錄'
    },
    {
      id: 'T02', type: '格式殘留', level: 'low', approval: false,
      test: t => /_x000D_/i.test(norm(t)),
      reason: 'Excel 換行格式殘留，不屬教師內容問題。',
      suggestion: '不退件｜輸出時自動清理格式'
    },
    {
      id: 'T03', type: '語意不明／可能相反', level: 'high', approval: true,
      test: t => has(t, '降低病患安全|降低病人安全|沒尚無法|即時向觀察|無法及時處理.*可以|可以.*無法'),
      reason: '句意可能相反或存在關鍵語意缺口，程式不應自行替教師決定原意。',
      suggestion: '建議退件｜請教師確認原意'
    },
    {
      id: 'T04', type: '紀錄表述風險', level: 'high', approval: true,
      test: t => has(t, '耍小聰明|心不在焉|懶惰|不用心|散漫|態度很差|學習態度被動|能力很差'),
      reason: '較強的負向標籤可能影響後續對學員的判讀；較適合改為可觀察行為。',
      suggestion: '建議退件｜調整為可觀察行為描述'
    },
    {
      id: 'T05', type: '前後敘述待確認', level: 'medium', approval: true,
      test: t => has(t, '(已|逐漸|明顯).{0,12}(改善|進步).{0,28}(時常|仍常|持續).{0,12}(遺漏|錯誤|無法|需要提醒)'),
      reason: '同一紀錄同時出現改善與持續性問題，需由人工確認目前實際程度。',
      suggestion: '待主管判斷｜是否需退件確認'
    }
  ];
  ...
  function managementSignals(row, t) {
    ...
    if (med) issues.push('給藥／病安事件');
    else if (safety) issues.push('病人安全');
  ...
  function managementDetail(t, issues) {
    ...
    if (issues.some(x => /給藥|病人安全/.test(x))) {
      ...
      if (!hasOutcome) {
        state = '事件訊號明確，但影響／處理資訊不足';
        question = '確認事件是否到達病人、影響、即時處置、制度處理及後續追蹤。';
      } else {
        question = '確認事件釐清結果、制度處理與改善措施是否持續落實。';
      }
    } else if (issues.some(x => /離職|轉調/.test(x))) {
      question = '確認人員異動狀態、輔導結案與必要交接。';
    } else if (issues.some(x => /請假/.test(x))) {
      question = '確認連續請假是否影響輔導進度，以及是否需要調整後續安排。';
    } else if (issues.some(x => /工作適應|情緒/.test(x))) {
      question = '確認目前支持措施、工作安排及下月是否仍需追蹤。';
    }
  ...
  function analyzeRecord(row, sourceRow) {
    ...
    const gap = keyContentGap(t, managementIssues);
    if (gap) {
      const gapRule = {
        id: 'T06', type: '關鍵內容不足', level: 'high', approval: true,
        reason: '紀錄已出現重要病安／事件訊號，但缺少足以完成管理判讀的影響或處理資訊。',
        suggestion: '建議退件｜補充必要事件與後續資訊'
      };
      ruleHits.push(gapRule.id);
      if (!primary || primary.level !== 'high') primary = gapRule;
    }
    ...
    return {
      sourceRow,
      caseId: `C-${String(sourceRow).padStart(2, '0')}`,
      row,
      cleanedText: t,
      textIssue,
      textType: primary?.type || '',
      textReason: primary?.reason || '',
      textSuggestion: primary?.suggestion || '',
      needsApproval,
      management,
      managementIssue: managementIssues.join('、'),
      managementState: management ? detail.state : '',
      managementQuestion: management ? detail.question : '',
      highlight: highlight.yes,
      highlightType: highlight.type,
      highlightReason: highlight.reason,
      highlightFeedback: highlight.feedback,
      confidence,
      ruleHits: [...new Set(ruleHits)].join('、'),
      excerpt: clip(t, 220)
    };
  }
